...
var count = 250
var samples = ee.Image.pixelLonLat().sample({region: bounds, numPixels: count, scale: 10000}).map(sampleFeatureToPoint)

// draw text
var scale = Map.getScale()
var samplesText = ee.ImageCollection(samples.map(function(f) { 
  var pos = ee.Geometry(f.geometry())
  var str = toString(pos.coordinates())
  return Text.draw(str, pos, scale) 
}))

// add to map
Map.addLayer(samples, {color:'eeee40'}, 'points')
Map.addLayer(samplesText.mosaic(), {}, 'coords')
