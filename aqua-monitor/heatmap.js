...
Map.addLayer(locations, {color:'ffff00'}, 'centers', false)
...
// var fc = ee.FeatureCollection('ft:1aQkcjiyP3QYywUcNJwvgBn4To5ujiJuMyF5d9BAA')
var fc = ee.FeatureCollection('ft:17TEfjvF14hKeDmSotkXrjYeplyT8O_SgRLuJFbYk')

var heatmap = fc
  .reduceToImage(['total_wate'], ee.Reducer.sum())
  .focal_max(4, 'circle')
  .convolve(ee.Kernel.gaussian(8, 3));

var heatmapColors = ['000000', '0000ff', 'ffffff']
Map.addLayer(heatmap, {min:0, max:5000000, opacity: 0.7, palette: heatmapColors}, 'heatmap (water)', true);

var heatmap = fc
  .reduceToImage(['total_land'], ee.Reducer.sum())
  .focal_max(2, 'circle')
  .convolve(ee.Kernel.gaussian(4, 2));

var heatmapColors = ['000000', '00ff00', 'ffffff']
Map.addLayer(heatmap, {min:0, max:5000000, opacity: 0.7, palette: heatmapColors}, 'heatmap (land)', true);
