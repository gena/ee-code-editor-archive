...
var app = function app() {
    ...
    var resolutionMin = 30;

    print(start + '..' + stop);

    var skip = ['ASTER T', 'ASTER',
    //'Landsat 8',
    'Landsat 7', 'Landsat 5', 'Landsat 4',
    //'Sentinel 2',
    //'PROBA-V 100m',
    //'PROBA-V 333m',
    //'MODIS Aqua MYD09GQ',
    //'MODIS Terra MOD09GQ',
    'Sentinel 1 VV', 'Sentinel 1 VH', // too noisy
    'Sentinel 1 VV+VH', 'Sentinel 1 HH+HV'];
    ...
    var images = ee.ImageCollection(ee.List(rendered).iterate(mergeCollection, ee.ImageCollection([]))).sort('system:time_start');

    print(images.size());

    var bounds = ee.Geometry(Map.getCenter()).buffer(Map.getScale() * 100, 100).bounds();
    ...
    var gallery = community.ImageGallery(images, bounds, g.rows, g.columns, g.options);
...
var aoi = /* color: d63000 */ee.Geometry.Polygon([[[-120.13781547546387, 39.37504697058432], [-120.13326644897461, 39.37929311183573], [-120.13824462890625, 39.38287559262373], [-120.13978944325117, 39.38479941281629], [-120.1413345336914, 39.389774667175914], [-120.14897346496582, 39.39037167040836], [-120.1517630200903, 39.38635855593795], [-120.16176223754883, 39.389907335002306], [-120.16674041013675, 39.38771853899016], [-120.17480850219727, 39.39083600272453], [-120.17789843439749, 39.391764637908786], [-120.1768684387207, 39.38712125767802], [-120.17077445983887, 39.38141608560946], [-120.1620302615708, 39.37926011043577], [-120.15843618830496, 39.37750176036189], [-120.16484137400715, 39.37584315724959], [-120.17287723584599, 39.37677189069475], [-120.17206192016602, 39.373454601030474], [-120.16356468200684, 39.36701840406483], [-120.15412330627441, 39.37060151475313], [-120.15176325507196, 39.37723632094731], [-120.14828681945801, 39.37703738145347], [-120.14463920832839, 39.37640716332671]]]),
...
Algorithms.Aster.detectWater = function (image, info, returnImage) {
    ...
    function addProperties(element) {
        return element.set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not().reduceRegion(ee.Reducer.sum(), analysisExtent, scale)).values().get(0)).set('cloud_pixels', ee.Dictionary(cloudMask.reduceRegion(ee.Reducer.sum(), analysisExtent, scale)).values().get(0))
...
Algorithms.AsterT.detectWater = function (image, info, returnImage) {
    ...
    function addProperties(element) {
        return element.set('system:time_start', image.get('system:time_start')).set('nodata_pixels', 0).set('cloud_pixels', 0).set('cloud_shadow_pixels', 0).set('snow_pixels', 0).set('water_threshold', th);
    }
...
Algorithms.Sentinel1.detectWater = function (image, info, returnImage) {
    ...
    function addProperties(element) {
        return element.set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not().reduceRegion(ee.Reducer.sum(), analysisExtent, scale)).values().get(0)).set('cloud_pixels', 0).set('cloud_shadow_pixels', 0).set('snow_pixels', 0).set('water_threshold', th);
    }
