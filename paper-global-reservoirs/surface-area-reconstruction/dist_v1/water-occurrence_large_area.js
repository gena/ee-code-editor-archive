...
var ned = ee.Image("USGS/NED"),
    aoi = /* color: #d63000 */ee.Geometry.Polygon(
        [[[29.06604766845703, -28.555576049185962],
          [29.07789374147592, -28.5516541050306],
          [29.081153869628906, -28.53205183048953],
          [29.06363543163684, -28.48452726902855],
          [29.054718017578125, -28.45533547672329],
          [29.056821114315426, -28.442768158403396],
          [29.06424510572242, -28.430276540328773],
          [29.063150381931337, -28.42621967945084],
          [29.05536139857759, -28.41468981225837],
          [29.058494567871094, -28.408916545005503],
          [29.042701721191406, -28.383547360197635],
          [29.007854823524895, -28.38294693209324],
          [28.97918701171875, -28.385963734648637],
          [28.979529754984696, -28.424686280372352],
          [28.982276916503906, -28.46537136397389],
          [28.976784594276978, -28.484231428870764],
          [28.973350524902344, -28.50520367835768],
          [28.994376065785673, -28.511014966013814],
          [29.02501290869145, -28.4981159833178],
          [29.021323416503947, -28.535896062336974],
          [29.046478271484375, -28.549544699103865]]]),
    aoiSudan = /* color: #ffc82d */ee.Geometry.Polygon(
        [[[34.27820721958278, 11.643505463931282],
          [34.31774662849489, 11.541659513498315],
          [34.3869006259099, 11.42074183391437],
          [34.75238430147874, 11.265992287507414],
          [34.7882080078125, 11.321173722096153],
          [34.716796875, 11.37368511744568],
          [34.617919921875, 11.525778716700001],
          [34.5849609375, 11.613228971431123],
          [34.5904541015625, 11.67106561977198],
          [34.556121826171875, 11.774603407627687],
          [34.446258544921875, 11.82030900306308],
          [34.377593994140625, 11.827029771365765],
          [34.322662353515625, 11.747714210698645]]]),
...
var app = function app() {
    ...
    var images = ee.ImageCollection(ee.List(results).get(0))
      .map(function(i) {
        return i.set('any', i.reduceRegion({reducer:ee.Reducer.anyNonZero(), geometry:aoi, scale: scale, maxPixels:maxPixels}).values().get(0))
    ...
    var results = collections.map(function (collection) {
        ...
        images = Algorithms.filter(images, collection, aoi, {
            skipEmpty: false,
            maxNoDataPixels: -1, // e.g. Landsat 7
    ...
    }).sum().reduceRegion({maxPixels: maxPixels, reducer: ee.Reducer.max(), geometry: aoi, scale: scale}).values().get(0));

    // compute water occurrence
    var waterOccurrence = waterImages.select(['water']).sum().divide(waterImages.select(['mask']).sum());
    print(ui.Chart.image.histogram(waterOccurrence, aoi, scale, 100));
...
Algorithms.filter = function (images, collection, geometry, options) {
    ...
    if (options.skipEmpty) {
        images = images.map(function (image) {
            var value = ee.Dictionary(image.select(collection.visual.bands[0])
              .reduceRegion(
                {maxPixels: maxPixels, reducer: ee.Reducer.firstNonNull(), geometry: geometry, scale: scale*3}
                )).values().get(0);
...
Algorithms.Aster.detectWater = function (image, info, returnImage) {
    ...
    if (returnImage) {
        return water.set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not()
          .reduceRegion(
            {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
            )).values().get(0)).set('cloud_pixels', ee.Dictionary(cloudMask
              .reduceRegion(
                {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
                )).values().get(0))
        ...
        .set('snow_pixels', ee.Dictionary(snowMask.reduceRegion(
          {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
          )).values().get(0)).set('water_threshold', th);
    }
    ...
    return ee.FeatureCollection(waterVector.copyProperties(image)).set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not()
      .reduceRegion(
        {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
        )).values().get(0)).set('cloud_pixels', 
          ee.Dictionary(cloudMask.reduceRegion({maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale})).values().get(0))
          .set('cloud_shadow_pixels', ee.Dictionary(cloudShadows
          .reduceRegion(
            {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
            )).values().get(0)).set('snow_pixels', ee.Dictionary(snowMask.reduceRegion({maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale})).values().get(0)).set('water_threshold', th);
};
...
Algorithms.AsterT.detectWater = function (image, info, returnImage) {
    ...
    var mode = ee.Number(ee.Dictionary(t.reduceRegion(
      {maxPixels: maxPixels, reducer: ee.Reducer.mode(), geometry: waterPermanentCenter, scale: errorMargin * 5}
      )).get('temp'));
...
Algorithms.Landsat.detectWater = function (image, info, returnImage) {
    ...
    if (returnImage) {
        return water.set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not().reduceRegion(
          {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
          )).values().get(0)).set('cloud_pixels', ee.Dictionary(cloudMask.reduceRegion(
            {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale})).values().get(0))
        //.set('cloud_shadow_pixels', ee.Dictionary(cloudShadows.reduceRegion(ee.Reducer.sum(), analysisExtent, scale)).values().get(0))
        .set('snow_pixels', ee.Dictionary(snowMask.reduceRegion(
        {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
        )).values().get(0)).set('water_threshold', th);
    ...
    return ee.FeatureCollection(waterVector.copyProperties(image)).set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not().reduceRegion(
      ...
      {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
      )).values().get(0)).set('cloud_shadow_pixels', ee.Dictionary(cloudShadows.reduceRegion(
          {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
          )).values().get(0)).set('snow_pixels', ee.Dictionary(snowMask.reduceRegion(
                      {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
                    )).values().get(0)).set('water_threshold', th);
};
...
Algorithms.Proba.detectWater = function (image, info, returnImage) {
    var waterScore = image.resample('bicubic').normalizedDifference(['red', 'nir']);

    return waterScore.gt(0)
};
...
Algorithms.Sentinel1.detectWater = function (image, info, returnImage) {
    ...
    if (returnImage) {
        return water.set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not().reduceRegion(
                        {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
        )).values().get(0)).set('cloud_pixels', 0).set('cloud_shadow_pixels', 0).set('snow_pixels', 0).set('water_threshold', th);
    }
...
Map.addLayer(jrc.select('occurrence'), {min:0, max:100, palette: Palettes.water}, 'JRC water occurrence', false)