...
var ned = ee.Image("USGS/NED"),
    ...
    aoi_small1 = /* color: #d63000 */ee.Geometry.Polygon(
        [[[31.8878173828125, 49.63517499335973],
          [31.797866821289062, 49.536795449984666],
          [32.09930419921875, 49.39622819171437],
          [32.21122741699219, 49.51138935408079],
          [32.20024108886719, 49.51406430165547],
          [32.18376159667969, 49.51763067086701],
          [32.15766906738281, 49.521642525379775],
          [32.14874267578125, 49.526545456153805],
          [32.08831787109375, 49.55417096223951],
          [32.02239990234375, 49.58934857766428],
          [31.959228515625, 49.61515904368315]]]);
...
var app = function app() {
    ...
    var results = collections.map(function (collection) {
        ...
        images = Algorithms.filter(images, collection, aoi, {
            skipEmpty: true,
            maxNoDataPixels: 100, // e.g. Landsat 7
            maxCloudPixels: 100,
            maxSnowPixels: 100
        });
    ...
    return ee.FeatureCollection(waterVector.copyProperties(image)).set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not()
      .reduceRegion(
        {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
          ...
          ).values().get(0)).set('cloud_shadow_pixels', 
        ee.Dictionary(cloudShadows
          .reduceRegion(
            {maxPixels: maxPixels, reducer: ee.Reducer.sum(), geometry: analysisExtent, scale: scale}
...
Algorithms.Landsat.onLayerAdd = function (image, info) {
    ...
    var results = computeThresholdUsingOtsu(i, scale, aoi, cannyTh, cannySigma, false, true);
    var th = results.threshold;
...
Algorithms.Sentinel2.onLayerAdd = function (image, info) {
    ...
    var th = computeThresholdUsingOtsu(i, scale, aoi, 0.3, 2, false, false);
