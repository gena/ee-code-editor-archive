...
var ned = ee.Image("USGS/NED"),
    ...
    occurrence = ee.Image("users/gena/water-occurrence-Prosser-Creek-2000-01-01_2017-04-01_Sterkfontein");
...
*/      
  ...
  var edgeMargin = 0.20 // probability margin 
  ...
  */
  ...
  var bad = occurrence.unmask(1, false).lt(occurrenceExpected)
    .multiply(occurrenceUpdatedRange.not())
    .multiply(i.unmask(0, true))
    .multiply(imageMask.not().or(cloudMask))
...
*/
  ...
  return ee.ImageCollection([
      ...
      // margin
      occurrenceUpdated.mask(occurrenceUpdated).visualize({min:0, max:1, opacity:0.7, palette:['f2f0f7', 'cbc9e2', '9e9ac8', '6a51a3']}),
      occurrenceEdge.visualize({palette:['6a51a3'], forceRgbOutput: true}),
...
/*
          ...
          //var edgeGradient = gradient.select(0).max(gradient.select(1)).mask(edge.focal_max(1)) 
            // .reproject(image.projection().scale(2, 2));
            
          var edgeGradient = image.mask(edge.focal_max(1)).reduceNeighborhood(ee.Reducer.min(), ee.Kernel.circle(2)).mask(edge)

          // take the highest gradieng percentiles only
          var percentile = 25
...
Algorithms.Landsat.onLayerAdd = function (image, info) {
    ...
    var waterImageVis = ee.Image(ee.Algorithms.If(
      ee.Algorithms.IsEqual(area, 0),
      ee.Image(),
      fillMissingWater(water, snow, clouds, mask, results.edges)
    ))
    Map.addLayer(waterImageVis, {}, 'water filled', false)
...
*/
    ...
    //var clipEdges = false;
    var clipEdges = true
    var results = computeThresholdUsingOtsu(image, scale, aoi, 0.6, 2, clipEdges, true, null, true);
    var th = results.threshold;
