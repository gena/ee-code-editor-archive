...
//var showGapFilling = true
var showGapFilling = false

var updateEdgesDensityMode = true
...
function computeThresholdUsingOtsu(image, scale, bounds, th, g, skipShort, weightGradient, minValue, useLowValues) {
    ...
    // clip image edges
    var mask = image.mask().gt(0).focal_min(ee.Number(scale), 'circle', 'meters');
    ...
    if(debug) {
      Map.addLayer(image, { }, 'edges (image)', false);
    ...
    // fall-back to mode update only
    if(updateEdgesDensityMode) {
...
*/        
      ...
      //if(useLowValues) {
        var occurrenceEdgeValue = occurrenceEdge.reduceRegion(ee.Reducer.intervalMean(75, 95), bounds, scale).values().get(0)
      //} else {
      //  var occurrenceEdgeValue = occurrenceEdge.reduceRegion(ee.Reducer.intervalMean(25, 55), bounds, scale).values().get(0)
      //}
      
      //skipShort = false // clip using density instead of skip short
      //weightGradient = false
  
      edge = ee.Image(ee.Algorithms.If(
        ee.Algorithms.IsEqual(occurrenceEdgeValue, null),
        edge,
        edge.multiply(occurrence.subtract(ee.Image.constant(occurrenceEdgeValue)).abs().lt(0.1)) // 10% around most probable
      ))
      
      if(debug) {
        // show histogram of density along edges (find mode)
        print('Edge density expected: ', occurrenceEdgeValue)
...
Algorithms.Landsat.onLayerAdd = function (image, info) {
    ...
    var i = waterScore
      .updateMask(cloudMask.not().and(snowMask.not())).clip(aoi);
