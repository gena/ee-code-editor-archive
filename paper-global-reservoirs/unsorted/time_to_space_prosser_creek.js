...
var dem = ee.Image("USGS/NED"),
    geometry = /* color: #d63000 */ee.Geometry.LineString(
        [[-120.17532348632812, 39.389774667175914],
         [-120.13704299926758, 39.369075397493006]]);
...
//var images = l8
  ...
  .filter(ee.Filter.dayOfYear(120, 250))
...
var gallery = function(images, sortProperty, waterOtsu) {
  ...
  var mosaic = imagesSorted
    ...
    // waterScore = waterScore.mask(mask)
    ...
    if(waterOtsu) {
      ...
      var cannySmoothingSigma = 1
      var o = computeThresholdUsingOtsu(waterScore, 30, bounds, cannyThreshold, cannySmoothingSigma, false, false, -0.1);
      th = o.threshold
    ...
    var str = ee.String('th=').cat(ee.Number(th).format('%.2f')).cat(',  t=').cat(i.date().format('YYYY-MM-dd'));
    ...
    return ee.ImageCollection.fromImages([
        ...
        i.select('cloud').mask(i.select('cloud').divide(80)).visualize({palette:['ffff00']}),
...
//Map.addLayer(gallery(images, 'system:start_time'), {}, 'sorted by time', true)
//Map.addLayer(gallery(images, 'system:start_time', true), {}, 'sorted by time (Otsu)', true)
