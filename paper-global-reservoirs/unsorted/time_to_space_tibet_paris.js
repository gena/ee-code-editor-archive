...
var dem = ee.Image("USGS/NED"),
    geometry = /* color: #98ff00 */ee.Geometry.LineString(
        [[9.92495811564811, 53.51817991400317],
         [9.95771905984202, 53.54825398116283]]);
...
var boundsOutline = ee.Image().paint(bounds, 1, 2).visualize({palette:['000000'], forceRgbOutput: true}).reproject('EPSG:3857', null, Map.getScale())
...
var gallery = function(images, sortProperty, waterOtsu) {
  ...
  var mosaic = imagesSorted
    ...
    if(waterOtsu) {
      //var o = computeThresholdUsingOtsu(waterScore, 30, bounds, 0.3, 0.5, false, false, -0.1);
      var o = computeThresholdUsingOtsu(waterScore, 30, bounds, 0.7, 0.7, false, false, -0.1);
      th = o.threshold
