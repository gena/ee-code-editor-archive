...
var idw_per_cell = function(grid_cell) {
  ...
  return idw_cell.clip(grid_cell)
}

// just show it for a single cell (test)
/*var firstCell = ee.Feature(grid.first())
Map.centerObject(firstCell, 8)

var result = idw_per_cell(firstCell).clip(firstCell)
Map.addLayer(result, {min:0, max:6}, 'first cell idw')

Map.addLayer(ee.FeatureCollection([firstCell]), {}, 'first cell')

Map.addLayer(dflow_ev.reduceToImage(['rp00100'],
  ee.Reducer.max()).focal_max(5).clip(firstCell),
  {min: 0, max: 6, palette: ['000000, 00FF00'], opacity: 0.5},
  'rp 100');
*/

grid = grid.filterBounds(dflow_ev)

Map.centerObject(grid, 8)

var interpolated_data = ee.ImageCollection(grid.map(idw_per_cell)).median();
Map.addLayer(interpolated_data, {min:4, max:6, palette:['0000ff','00ff00']}, 'idw')
