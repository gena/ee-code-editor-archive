...
//Map.addLayer(images.select(0).count(), {min:0, max:2500, palette:['ff0000', '00ff00'], opacity:0.5})  
  //print(images.aggregate_count('system:id'))  
...
function addPercentileLayers(percentile, water_color, water_edge_color, visible) {
  ...
  Map.addLayer(l8.reduce(ee.Reducer.percentile([percentile])).rename(STD_NAMES)
    .select(['swir1', 'nir', 'green']), visProps, 'swir1_nir_green ' + percentile + '% L8', visible)

  Map.addLayer(l7.reduce(ee.Reducer.percentile([percentile])).rename(STD_NAMES)
    .select(['swir1', 'nir', 'green']), visProps, 'swir1_nir_green ' + percentile + '% L7', visible)
