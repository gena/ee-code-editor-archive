...
function Proba() {
  ...
  proba = ee.ImageCollection(proba.merge(proba330))
    .filterDate(start, stop)
    .map(function(i) {
      var any = i.select(0).reduceRegion(ee.Reducer.anyNonZero(), bounds).values().get(0)

      return i.visualize({min:30, max:1000, bands: ['SWIR', 'NIR', 'RED']})
        .set('any', any)
        .set('system:time_start', i.get('system:time_start'))
    }).filter(ee.Filter.eq('any', 1))
    

  var videoFrames = ee.ImageCollection(modis.merge(proba)).sort('system:time_start')
...
function Modis() {
  ...
  var modis = ee.ImageCollection(mod.merge(myd))
    ...
    .map(function(i) {
      var any = ee.Image(i).reduceRegion(ee.Reducer.anyNonZero(), bounds).values().get(0)
      return i
        .visualize({min:800, max:5000, bands:['sur_refl_b01', 'sur_refl_b02', 'sur_refl_b01']})
        .set('has_values', any)
        .set('system:time_start', i.get('system:time_start'))
    })
