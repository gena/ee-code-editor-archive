function app() {
  ...
  var aoi = geometry.difference(geometry2)
  ...
  // fails at high zoom levels (uses pixels), enable reproject
  // some artifacts of fastDistanceTransform() are seen for smaller multiliers, like 0.1
  d = d
    .reproject('EPSG:3857', null, step*0.5) 
    .resample('bicubic')
  ...
  Map.addLayer(d, {min:0, max:100000}, 'distance', false)
  
  Map.addLayer(aspect, {min:0, max:2*Math.PI, palette:colorsRainbow}, 'aspect', true)

  Map.addLayer(slope.mask(slope.subtract(1.5).multiply(-1)), {palette:['000000', 'ffeda0'], min:0.3, max:1}, 'slope', true)
}


// geoms, do not show geometry layers on run
