...
community.Charts.Scatter = function(map, bounds) {
  ...
  this.targetScale = 60 // target projection size
  this.samplingScale = 15
}
...
community.Charts.Scatter.prototype.plotArea = function() {
  ...
  this.map.addLayer(bg.focal_max(1, 'square'), {palette:['000000']}, ' area (black)', true, 0.85)
  this.map.addLayer(bg.focal_max(1, 'square'), {palette:['ffffff']}, ' area (white)', true, 0.85)
} 
...
community.Charts.Scatter.prototype.renderValues = function(collection, region, axesBands, renderBands, name, opt_visible) {
  ...
  print('Collection size:', collection)
  ...
  var image = features
    .reduceToImage(renderBands, ee.Reducer.mean())
    // .reduceToImage(renderBands, ee.Reducer.min())
    //.reduceToImage(renderBands, ee.Reducer.sum())
...
community.Charts.Scatter.prototype.plotValues = function(collection, region, axesBands, renderBands, name, opt_visible) {
  ...
  <RasterSymbolizer>\
    <ColorMap extended="true" >\
      ...
      <ColorMapEntry color="#000000" quantity="-0.01" label="-1"/>\
      <ColorMapEntry color="#ff0000" quantity="0.0" label="-1"/>\
      <ColorMapEntry color="#000011" quantity="0.01" label="-1"/>\
      <ColorMapEntry color="#0000ff" quantity="1.0" label="-1"/>\
    </ColorMap>\
  </RasterSymbolizer>';

  // this.map.addLayer(image.mask(image.neq(0)).sldStyle(ndwiStyle), {}, name + ' values', visible)
  
  var paletteDiverging = ['9e0142', 'd53e4f', 'f46d43', 'fdae61', 'fee08b', 'ffffbf', 'e6f598', 'abdda4', '66c2a5', '3288bd', '5e4fa2']
  this.map.addLayer(image, {min:-1, max:1, palette:paletteDiverging}, name + ' values', visible)
}
...
info.map(function(i) {
  ...
  var axesBands = ['green', 'swir1']
  //var axesBands = ['green', 'nir']
  var renderBands = ['ndwi']
  
  i.collection = i.collection.map(function(i) {
    return i.addBands(i.normalizedDifference(['green', 'swir1']).rename('ndwi'))
    //return i.addBands(i.normalizedDifference(['green', 'nir']).rename('ndwi'))
  })
  
  i.collection = i.collection.select(['green', 'swir1', 'ndwi'])
  //i.collection = i.collection.select(['green', 'nir', 'ndwi'])
