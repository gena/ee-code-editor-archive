...
var L8RAW = ee.ImageCollection("LANDSAT/LC8_L1T"),
    L7RAW = ee.ImageCollection("LANDSAT/LE7_L1T"),
...
//Edited 4-12-2016- Christopher Anderson
//Stanford University
//cbanders@stanford.edu
//Pulled from https://groups.google.com/d/topic/google-earth-engine-developers/b5UJOdI1PI4/discussion
...
var years = [2012];//Specify years- can enter a list or use range() function
var compositingPeriod = 5;//Number of years to include in the composite
var startJulian = 1;//Start julian date- supports wrapping
var endJulian = 360;//End julian date
...
var possibleSensors = ee.List(['L8']); //Specify which sensors to pull from- supports L5,L7, and L8
...
var collection_dict = {L8: 'LC8_L1T',
                         L7: 'LE7_L1T',
                         L5: 'LT5_L1T',
                         L4: 'LT4_L1T'
  };
...
var vizParamsCO1 = {'min': 0.05,'max': [0.3,0.4,0.4],   'bands':'swir1,nir,red'};
...
/////////////////////////////////////////////////////////////////
//Functions to mask clouds, ensure data exists in every band, and defringe images
...
function getImage(year,compositingPeriod, startJulian,endJulian,shadowLookBack,shadowLookForward,singleImageMode){
  ...
  ls = ls.filterDate(startDate,endDate);
  ...
  //Add the raw landsat data to the stack prior to compositing
  var mergeBands = function(element) {
    return ee.Image.cat(element.get('primary'), element.get('secondary'));
  };
  
  //Set filter to match up all scenes using LANDSAT ID
  ...
  var coll = ee.FeatureCollection(ee.Join.inner().apply(ls,
            ee.ImageCollection(collection_dict['L8'])
            .filterBounds(fB)
            .filter(ee.Filter.calendarRange(startJulian,endJulian)),
            filter));//.map(mergeBands);

  //Print out variables for troubleshooting
  print('LS collection:');
  print(ls);
  print('coll collection');
  print(coll);
  coll = ee.ImageCollection(coll.map(mergeBands));
  print('coll after merge');
  print(coll);
  var composite = ls.reduce(reducer);//.select(bandNumbers,bandNames)
  coll = coll.reduce(reducer);
  ...
  //                //ls.sort('system:time_start',true).reduce(ee.Reducer.first())));
  ...
  var fullName = saName+' '+y1Image.toString()+'-' +y2Image.toString()+' '+startJulian.toString()+'-'+endJulian.toString();
  ...
  //Append the metadata if it already exists
   ...
   if(metadataFC === ''){
      ...
      metadataFC = metadataFC.merge(f);
    }
   
   //Add to map and export composite if compositing
   if(singleImageMode === false){
    addToMap(composite,vizParams,year.toString() + '_composite');
    Export.image(toExport,fullName,{
      'scale':ps,
      'maxPixels':1e13,
      'crs':crs,
      'region': region
