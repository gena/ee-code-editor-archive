...
var point = /* color: #00ffff */ee.Geometry.Point([-120.03838062286377, 39.14683645090244]),
    box = /* color: #bf04c2 */ee.Geometry.LineString(
...
info.map(function(i) {
  ...
  Map.addLayer(image, {bands: ['SWIR', 'NIR', 'BLUE'], min:0, max:0.5}, i.name + ' first image')
...
info.map(function(i) {
  ...
  var axesBands = ['BLUE', 'SWIR']
  var renderBands = ['SWIR', 'NIR', 'BLUE']
  i.collection = i.collection.select(renderBands)

  var stepYears = 1
  var stepMonths = 3
  var years = ee.List.sequence(2015, 2016, stepYears).getInfo()
