...
var geometry = /* color: #d63000 */ee.Geometry.MultiPoint(),
    textCenter = /* color: #98ff00 */ee.Geometry.Point([-79.81790542602533, -2.2188551212387027]);
...
// compute position of the text relative to image center
...
var textPos = textCenter.coordinates()

Map.addLayer(textCenter)

function annotate(images, params) {
  return images.map(function(i) {
    ...
    // draw text
    var pos = translate(textPos, 0, 0)
    var s = ee.String('DATE: ').cat(ee.String(i.get('DATE_ACQUIRED')))
    var textDate = Text.draw(s, pos, scale, props)
  
    pos = translate(textPos, 0, dy)
    s = ee.String('TIME: ').cat(ee.String(i.get('SCENE_CENTER_TIME')).slice(0, 5))
    var textTime = Text.draw(s, pos, scale, props)

    pos = translate(textPos, 0, dy * 2)
    s = ee.String('ID: ').cat(ee.String(i.get('LANDSAT_SCENE_ID')))
    var textCloud = Text.draw(s, pos, scale, props)
  
    var image = i.visualize(params)
    ...
    return ee.ImageCollection([
      ...
      textDate, 
      textTime, 
      textCloud]).mosaic() // merge results
  }) 
}
...
var name = 'Guayaquil_Ecuador_swir1_nir_green'
var params = {min: 0.05, max: 0.4, gamma:1.2}
var l8 = ee.ImageCollection('LANDSAT/LC8_L1T_TOA').select([5,4,2])
var l7 = ee.ImageCollection('LANDSAT/LE7_L1T_TOA').select([4,3,1])
var l5 = ee.ImageCollection('LANDSAT/LT5_L1T_TOA').select([4,3,1])
/*
var name = 'Brahmaputra_red_green_blue'
var params = {min: 0.05, max: [0.45,0.45, 0.5], gamma:1.2}
var l8 = ee.ImageCollection('LANDSAT/LC8_L1T_TOA').select([3,2,1]); 
var l7 = ee.ImageCollection('LANDSAT/LE7_L1T_TOA').select([2,1,0]);
var l5 = ee.ImageCollection('LANDSAT/LT5_L1T_TOA').select([2,1,0]);
*/
...
// convert image collection to image + annotations (text, frame)
var videoFrames = annotate(images, params)

var bounds = Map.getBounds(true)
//var bounds = ee.Image(images.first()).select(0).geometry().bounds()
...
// add first 5 frames (debugging)
var count = 5
var list = videoFrames.toList(count, 0)

for(var i = 0; i < count; i++) {
  Map.addLayer(ee.Image(list.get(i)), {}, 'frame ' + i.toString())
}
