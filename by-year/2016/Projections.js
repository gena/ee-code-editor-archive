...
/*
// show as WGS84
var j = i.changeProj(ee.Projection('EPSG:4326'), ee.Projection('EPSG:3857').scale(100000, 100000))
var label = Text.draw('WGS84', ee.Geometry.Point(0, -80), Map.getScale()*5).visualize({forceRgbOutput: true})
Map.addLayer(ee.ImageCollection([ee.Image(0).visualize({palette:['ffffff']}), j, label]).mosaic(), {}, 'dem (WGS84)')
*/

// show as Robinson
var worldRobinson = ee.Projection('PROJCS["World_Robinson",'
+ 'GEOGCS["gcs",DATUM["d",SPHEROID["s",6378137,298.257223563]],'
+ 'PRIMEM["Greenwich",0],UNIT["Degree",0.017453292519943295]],PROJECTION["Robinson"],UNIT["Meter",1]]')

var j = dem.reproject(worldRobinson.scale(0.1, 0.1), null, 100000)
  .changeProj(ee.Projection('EPSG:4326'), ee.Projection('EPSG:3857').scale(0.00001, 0.00001))
Map.addLayer(j)
print(j)
...
var label = Text.draw('Robinson', ee.Geometry.Point(0, -80), Map.getScale()).visualize({forceRgbOutput: true})

//.changeProj(ee.Projection('EPSG:4326'), ee.Projection('EPSG:3857').scale(0.1, 0.1))
//Map.addLayer(ee.ImageCollection([ee.Image(0).visualize({palette:['ffffff']}), j, label]).mosaic(), {}, 'dem (Robinson)')
