...
// a single image.
var summer = temperature.filterDate('2014-06-21', '2014-09-22')
    .reduce(ee.Reducer.mean())
    .select([0], ['summer']);
var winter = temperature.filterDate('2013-12-21', '2014-03-19')
    .reduce(ee.Reducer.mean())
    .select([0], ['winter']);
...
Map.addLayer(elevation, {min: 500, max: 0});
Map.addLayer(transect, {color: 'FF0000'});



// add sampling points
// add sampling points along the transect
...
var segments = transect.cutLines(distances.toList().get(0)).geometries();

var points = segments.zip(distances.toList().get(0)).map(function(s) {
  var segment = ee.List(s).get(0);
  var offset = ee.List(s).get(1)
  return makePointFeature(ee.Geometry(segment).coordinates().get(0), offset)
})

var fc = ee.FeatureCollection(points)
Map.addLayer(fc)
