...
var info = [
  ...
  },
  {
    name: 'SR PROBA-V',
    assets: [
      { id: 'VITO/PROBAV/S1_TOC_333M', bands: ['SWIR', 'NIR', 'BLUE'], multiplier: 0.0005 },
      { id: 'VITO/PROBAV/S1_TOC_100M', bands: ['SWIR', 'NIR', 'BLUE'], multiplier: 0.0005 },
    ]
  },
  {
    name: 'SR MODIS',
    assets: [
      { id: 'MODIS/MOD09GA', bands: ['sur_refl_b01', 'sur_refl_b02', 'sur_refl_b03', 'sur_refl_b04', 'sur_refl_b05', 'sur_refl_b06', 'sur_refl_b07'], multiplier: 0.0001 },
      { id: 'MODIS/MYD09GA', bands: ['sur_refl_b01', 'sur_refl_b02', 'sur_refl_b03', 'sur_refl_b04', 'sur_refl_b05', 'sur_refl_b06', 'sur_refl_b07'], multiplier: 0.0001 },
    ]
  },
  {
    name: 'DN',
...
info.map(function(i) {
  ...
  Map.addLayer(image, {bands: ['sur_refl_b04', 'sur_refl_b03', 'sur_refl_b02'], min:0, max:0.5}, i.name + ' first image')
...
info.map(function(i) {
  ...
  //i.collection = i.collection.select(['swir1', 'nir', 'green'])
  var axesBands = ['sur_refl_b06', 'sur_refl_b04']
  var renderBands = ['sur_refl_b04', 'sur_refl_b03', 'sur_refl_b02']
  i.collection = i.collection.select(renderBands)

  var stepYears = 1
  var stepMonths = 6
  var years = ee.List.sequence(2010, 2016, stepYears).getInfo()
  var months = ee.List.sequence(1, 12, stepMonths).getInfo()
  
  // add layer
  years.map(function(y) {
    months.map(function(m) {
      var start = ee.Date.fromYMD(y, m, 1)
      var stop = start.advance(stepMonths, 'month')
      plot.plotValues(i.collection.filterDate(start, stop), samplingArea, axesBands, renderBands, start.format('YYYY-MM-dd').getInfo() + ' - ' + stop.format('YYYY-MM-dd').getInfo() + ' ' + i.name + ' green vs swir1')
    })
  })
