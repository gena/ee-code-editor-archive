...
// cases = cases.merge(covid19_nl)

// throw(0)

var lastUpdateStr = 'Last dashboard update: '

var dateNew = '2020-03-25'
...
cases = cases.filter(ee.Filter.notNull(['deaths']))
...
map.onClick(function(pt) {
  ...
  layerSelection.setEeObject(ee.FeatureCollection([selection]))
  
  var prev = null
  
  if(currentDate != dates[0]) {
    // JHU is buggy, locations change :()
    // var selectionAll = cases.filterBounds(selection.geometry())
...
var blob = ee.Blob('gs://deltares-video-map/covid19_aggregated.json')
...
blob.string().evaluate(function(str) {
  ...
  dates.map(function(date) {
    ...
    var layer = ui.Map.CloudStorageLayer({
      ...
      path: 'corona-virus/v3/corona-virus-' + date, 
      maxZoom: 6, 
...
function updateChartOptions() {
  ...
  if(screenWidth <= 800) {
    ...
    labelRegion.style().set({'fontSize': '12px'})
    labelConfirmed.style().set({'fontSize': '12px'})
    labelRecovered.style().set({'fontSize': '12px'})
    labelDeaths.style().set({'fontSize': '12px'})
    ...
    labelDeathRate.style().set({'fontSize': '12px'})
    ...
    labelRegion.style().set({'fontSize': '24px'})
    labelConfirmed.style().set({'fontSize': '24px'})
    labelRecovered.style().set({'fontSize': '24px'})
    labelDeaths.style().set({'fontSize': '24px'})
    ...
    labelDeathRate.style().set({'fontSize': '24px'})
    panelAll.style().set({width: '330px'})
...
function render(features, regional, dt) {
    // large aggregated
    var imageConfirmed = style.Feature.linear(features.filter(ee.Filter.gt('confirmed', 0)), 'conf_r',
        { palette: paletteConfirmed, pointSizeMin: 1, pointSizeMax: 50, width: 1, opacity: 0.5, valueMin: 1, valueMax: confirmedMax_r })  
    
    var imageDeathsAndRecovered = style.Feature.linear(features.filter(ee.Filter.gt('recovered', 0)), 'dr_r',
        { palette: paletteRecovered, pointSizeMin: 1, pointSizeMax: 50, width: 1, opacity: 0.5, valueMin: 1, valueMax: confirmedMax_r })  
  
    var imageDeaths = style.Feature.linear(features.filter(ee.Filter.gt('deaths', 0)), 'deaths_r',
        { palette: paletteDeaths, pointSizeMin: 1, pointSizeMax: 50, width: 1, opacity: 0.5, valueMin: 1, valueMax: confirmedMax_r })  
  ...
  if(regional) {
    ...
    function show(featuresValue) {
      ...
      var imageConfirmed = style.Feature.linear(results, 'value_r',
          { palette: p, pointSizeMin: 1, pointSizeMax: 50, width: 1, opacity: 0.25, opacityLine: 0.5, valueMin: 1, valueMax: confirmedMax_r })  
...
function exportNew() {
  var casesNew = ee.FeatureCollection("users/gena/covid19/covid19_new")
  
  // var casesNew = cases

  casesNew = casesNew.filter(ee.Filter.eq('dt', dateNew))
  
  var regional = false
  var image = render(casesNew, regional, dateNew)
  exportTiles(image, dateNew, 'v3', 0, 6)

  map.addLayer(image, {}, 'corona-virus-' + dateNew)

  var regional = true
  var image = render(casesNew, regional, dateNew)
  exportTiles(image, dateNew, 'v3_regional', 6, 8)
...
function exportAll() {
  dates.map(function(dt) {
    ...
    exportTiles(image, dt, 'v3', 0, 6)
    
    var regional = true
    var image = render(casesDate, regional, dt)
    exportTiles(image, dt, 'v3_regional', 6, 8)
  })
}
