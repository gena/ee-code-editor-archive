...
function app() {
  ...
  var imageLandsat8 = ee.Image(l8.filterBounds(Map.getBounds(true)).toList(1,5).get(0)).select(['B6', 'B5', 'B3'])
  ...
  Map.centerObject(imageGallery.region, 14)
  ...
  Export.image.toDrive({
    image: ee.ImageCollection.fromImages([
        bg.visualize({palette:['ffffff'], forceRgbOutput: true}), 
        imageGallery.image, 
        labelGallery.image, 
        dateGallery.image, 
        frame, 
    ...
    fileNamePrefix: 'ee-figure', 
    scale: Map.getScale(), 
    crs: 'EPSG:3857',
    region: imageGallery.region.buffer(50*Map.getScale())})
}
