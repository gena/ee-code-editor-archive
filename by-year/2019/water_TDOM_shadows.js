...
Map.addLayer(s2sMosaicCloudsProjectedDark.map(function(i) {
  var ndwi = i.normalizedDifference(['green', 'nir'])
  
  ndwi = ndwi
        .where(ndwi.gte(ndwiMax), ndwiMax)
        .where(ndwi.lte(ndwiMin), ndwiMin)
        
  return ndwi
}).mean(), {min:ndwiMin, max:ndwiMax, palette: Palettes.water}, 'water occurrence')
