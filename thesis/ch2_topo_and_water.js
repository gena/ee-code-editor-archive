...
var end = ee.Date.fromYMD(2016,1,1);
...
var imgcoll_fall = getLandsat(start, end, 150, 250, selpoint, 50);
...
function computeThresholdUsingOtsu(image, scale, bounds, th, g, skipShort, weightGradient, minValue, usePrior) {
    ...
    if (weightGradient) {
        ...
        var p85 = ee.Number(ee.Dictionary(edgeGradient.reduceRegion(ee.Reducer.percentile([85]), bounds, scale)).values().get(0));
...
var gallery = function(images, sortProperty, rows, columns, waterOtsu, waterBands, correctTopography, handOnly, handMask, usePrior, showWaterIndex, fillGaps, showRGB, skipThresholds, skipWaterEdge) {
  ...
  var mosaic = imagesSorted
    ...
    if(waterOtsu) {
      var o = computeThresholdUsingOtsu(waterScore, 30, bounds, 0.4, 0.5, false, true, -0.1, usePrior);
      //var o = computeThresholdUsingOtsu(waterScore, 30, bounds, 0.4, 0.5, false, false, 0.0, usePrior);
      //var o = computeThresholdUsingOtsu(waterScore, 30, bounds, 0.7, 0.5, false, false, 0.0, usePrior);
      th = o.threshold
