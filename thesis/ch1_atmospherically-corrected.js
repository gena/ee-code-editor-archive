...
var locationFrameArea = /* color: #d63000 */ee.Geometry.LineString(
        [[-120.21780967712397, 39.51417242538997],
         [-120.03619194030756, 39.353414180458884]]),
    locationScalebar = /* color: #98ff00 */ee.Geometry.LineString(
        [[-120.21604138009491, 39.33801040536393],
         [-120.17233944260892, 39.33754602536381]]),
...
function app() {
  ...
  var clipArea = locationFrameArea.bounds(1).union(locationFrameArea2.bounds(1), 1)
  Map.centerObject(clipArea, 12)

  // images
  l5_sr = l5_sr.filterDate('2000-05-01', '2001-05-01')
  l5 = l5.filterDate('2000-05-01', '2001-05-01')
  
  Map.addLayer(l5_sr.select(0).count(), {min:0, max:100}, 'count SR', false)
  Map.addLayer(l5.select(0).count(), {min:0, max:100}, 'count', false)

  var imageSR = ee.Image(l5_sr.filterBounds(locationFrameArea).first()).divide(10000)
    .reproject('EPSG:3857')

  Map.addLayer(imageSR, {bands:['B3', 'B2', 'B1'], min:0.03, max:0.2, gamma: 1.5}, 'SR RGB')

  imageSR = imageSR
    .translate(-dx, 0, 'meters').clip(locationFrameArea2.bounds(1))
  Map.addLayer(imageSR, {bands:['B5', 'B4', 'B2'], min:0.03, max:0.3}, 'SR')
  
  Map.addLayer(imageSR, {bands:['B3', 'B2', 'B1'], min:0.03, max:0.2, gamma: 1.5}, 'SR RGB')
  
  var imageTOA = ee.Image(l5.filterBounds(locationFrameArea).first())
    .clip(locationFrameArea.bounds(1))
  Map.addLayer(imageTOA, {bands:['B5', 'B4', 'B2'], min:0.03, max:0.3}, 'TOA')

  Map.addLayer(imageTOA, {bands:['B3', 'B2', 'B1'], min:0.03, max:0.2, gamma: 1.5}, 'TOA RGB')
  ...
  var imageSR = ee.Image(l5_sr.filterBounds(locationFrameArea).first())
    .reproject('EPSG:3857')
    .translate(-dx, 0, 'meters').clip(locationFrameArea2.bounds(1))
  var fmask = imageSR.select('cfmask')
  Map.addLayer(fmask.mask(fmask.gt(0)), {palette: ['0000ff', 'ffff00', '00ffff', 'ff0000'], min:1, max:4}, 'SR cfmask')
  ...
  var frame = new Frame(locationFrameArea.bounds(1), {steps:4, size:3, format: '%.2f', showLeft: true, showTop: true}).draw()
