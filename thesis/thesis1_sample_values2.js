...
var region = ee.Geometry.Polygon(
        [[[-120.15180587768555, 39.37554457862171],
          [-120.14723539352417, 39.3743669005306],
          [-120.14463901519775, 39.3809516906138],
          [-120.14933824539185, 39.381830721387516]]]).bounds();
...
function addTemp(i) {
  return i.set('temp', i.select('temp').reduceRegion(reducer, region).values().get(0))
}

function addSwir(i) {
  return i.set('swir1', i.select('swir1').reduceRegion(reducer, region).values().get(0))
}

function addGreen(i) {
  return i
    .set('green_min', i.select('green').reduceRegion(ee.Reducer.min(), region).values().get(0))
    .set('green_max', i.select('green').reduceRegion(ee.Reducer.max(), region).values().get(0))
}


function addNir(i) {
  return i.set('nir', i.select('nir').reduceRegion(reducer, region).values().get(0))
}

function addBlue(i) {
  return i.set('blue', i.select('blue').reduceRegion(reducer, region).values().get(0))
}

function addNdsi(i) {
  return i.set('ndsi', i.normalizedDifference(['green', 'swir1']).reduceRegion(reducer, region).values().get(0))
}

function addNdwi(i) {
  return i.set('ndwi', i.normalizedDifference(['green', 'nir']).reduceRegion(reducer, region).values().get(0))
}

function addH(i) {
  var hsv = i.select(['swir1', 'nir', 'green']).rgbToHsv()
  var v = hsv.select('hue').multiply(hsv.select('value')).reduceRegion(reducer, region).values().get(0)
  return i.set('HSV', v)
}
...
var g = gallery(images.filter(ee.Filter.lte('green_max', greenTh)).sort('green_min'), region, rows, columns)
var image = g.image
Map.addLayer(image, {bands: ['swir1', 'nir', 'green'], min: 0.05, max: 0.4}, 'green_max <= ' + greenTh);

var g = gallery(images.filter(ee.Filter.gt('green_min', greenTh)).sort('green_min'), region, rows, columns)
var image = g.image
Map.addLayer(image, {bands: ['swir1', 'nir', 'green'], min: 0.05, max: 0.4}, 'green_min > ' + greenTh);

var g = gallery(images.filter(ee.Filter.and(
    ee.Filter.gt('green_max', greenTh),
    ee.Filter.lte('green_min', greenTh)
)), region, rows, columns)
var image = g.image
Map.addLayer(image, {bands: ['swir1', 'nir', 'green'], min: 0.05, max: 0.4}, 'green_min <= ' + greenTh + ' and green_max > ' + greenTh);
