...
var h = ee.ImageCollection("EO1/HYPERION"),
    ...
    pt1 = /* color: #d63000 */ee.Geometry.Point([1.037607192993164, 44.085118968213656]),
    image_area = /* color: #98ff00 */ee.Geometry.LineString(
        [[1.0256767272949219, 44.09275314803795],
         [1.0474777221679688, 44.082149286997534]]);
...
function app() {
  ...
  waterBandsPairs.map(function(waterBands) {
    ...
    Map.addLayer(waterEdge.mask(waterEdge), {palette:[color]}, 'water edge' + suffix, true);
  })
  ...
  var image = g.image
  Map.addLayer(image, {}, 'gallery (B044 < 0.217)', false);
